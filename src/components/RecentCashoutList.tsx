  ...
  return (
    <Card>
      <CardHeader className="gap-4 sm:flex-row sm:items-end sm:justify-between">
        <SectionHeading eyebrow="Register Log" title="Expense List" />
      ...
      <CardContent>
        <div className="grid gap-3">
          {cashouts.length === 0 && <p className="text-sm font-medium text-muted-foreground">No expenses recorded for this date.</p>}
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {formatDisplayDate(cashout.date)} | {cashout.category} | {cashout.paymentMode}
                </p>
                {cashout.notes ? <p className="text-sm text-muted-foreground">{cashout.notes}</p> : null}
              </div>
