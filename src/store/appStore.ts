...
export function useAppStore() {
  ...
  useEffect(() => {
    ...
    function handleSubscriptionError(error: unknown) {
      ...
      if (
        ...
        setAuthError('Internet connection required to load the workspace.')
        return
      }

      setAuthError('Your account signed in, but the workspace could not be loaded. Please sign in again or contact the owner.')
  ...
  return {
    ...
    currentUser,
    hasAuthenticatedSession: Boolean(authUser),
    hasWorkspaceAccess: canStartSubscriptions,
    dailyCashouts,
    data: financeData,
    deleteCashTransferEntry: actions.deleteCashTransferEntry,
    deleteDailyCashoutEntry: actions.deleteDailyCashoutEntry,
    deleteExpenseEntry: actions.deleteExpenseEntry,
